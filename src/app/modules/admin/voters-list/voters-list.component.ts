...
export class VotersListComponent implements AfterViewInit {
  ...
  constructor(private eventsService: EventsService, private backend: BackendService) {}

  ngAfterViewInit() {
    this.voters$ = this.backend.getVoters(this.eventsService.getSelectedEvent()._id).pipe(
      map((voters) => voters.sort()),
      shareReplay(1)
    );

    this.winners$ = this.voters$.pipe(
      map((voters) => [...voters]),
      switchMap((votersCopy) =>
        this.nextWinner$.pipe(
          map(() => {
